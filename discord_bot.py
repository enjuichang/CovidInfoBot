    ...
    if lokiResultDICT:
        if client.user.id not in mscDICT:    # 判斷 User 是否為第一輪對話
            mscDICT[client.user.id] = { 
                                        ...
                                        "vaccine_stock": {},
                                        # "inquiry_type": "",
                                        "inquiry_type" : "side_effect",
                                        "completed": False}
        print(" ")
        print(lokiResultDICT)
        for k in lokiResultDICT:
            # print(k)    # 將 Loki Intent 的結果，存進 Global mscDICT 變數，可替換成 Database。
            # if k == "inquiry_type":
            #     mscDICT[client.user.id]["inquiry_type"] = lokiResultDICT["inquiry_type"]
            if k == "side_effect":
                # print(lokiResultDICT["side_effect"])
        ...
        if mscDICT[client.user.id]["inquiry_type"] == "side_effect" and replySTR == "":   
            if "vaccine_shot" not in mscDICT[client.user.id]["side_effect"]:
                ...
                replySTR = "請問您要詢問[{}]疫苗的甚麼資訊呢？".format(mscDICT[client.user.id]["vaccine_stock"]["vaccine_shot"])        

        ### severe_side_effect 多輪對話的問句 ###
        if mscDICT[client.user.id]["inquiry_type"] == "severe_side_effect" and replySTR == "":   
            if "vaccine_shot" not in mscDICT[client.user.id]["severe_side_effect"]:
                replySTR = "請問您想知道哪個廠牌的疫苗資訊？"
            elif "severe_side_effect" not in mscDICT[client.user.id]["severe_side_effect"]:
                replySTR = "請問您要詢問[{}]疫苗的甚麼資訊呢？".format(mscDICT[client.user.id]["vaccine_stock"]["vaccine_shot"]) 
